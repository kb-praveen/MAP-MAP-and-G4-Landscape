...
# 2. SETUP & PATHS
# ==========================================
tree_path    <- "Path/G4/G4_analysis/Species.nwk"
protein_path <- "/Users/praveenbishnoi/Downloads/CGGBP1_percent_identity_human.tsv"

fig_dir      <- "Path/G4/G4_revision/Absolute_GC_analysis"
tab_dir      <- "Path/G4/G4_revision/Absolute_GC_analysis"
...
df_genomes_raw <- read.delim("Path/G4/G4_analysis/Metadata/Genomes.tsv", header=TRUE, sep="\t", check.names=FALSE)
df_promots_raw <- read.delim("Path/G4/G4_analysis/Metadata/Promoters.tsv", header=TRUE, sep="\t", check.names=FALSE)
...
# ==========================================
# 4. PLOT ENGINE (SIDE-BY-SIDE MATRIX GRID)
# ==========================================
...
for (task in trait_tasks) {
  ...
  for (env in env_config) {
    ...
    # Hide extreme visual outlier point safely from layout window bounds
    ...
    # Panel A: Raw Identity Plot (Global = Dashed, Clades = Solid)
    p_corr <- ggplot(df_plot, aes(x = .data[[curr_x]], y = .data[[target_y]])) +
      ...
      theme_nature +
      labs(x = task$label, y = "CGGBP1 % Identity (vs H. sapiens)")
    
    # Panel B: PIC Space Identity Plot (Global = Solid, Clades = Dashed)
    p_pic <- ggplot(df_pic_plot, aes(x = px, y = py)) +
      ...
      theme_nature + 
      labs(x = paste0("Contrast: ", task$label), y = "Contrast: CGGBP1 % Identity")
    
    composite_side_by_side <- p_corr + p_pic + plot_layout(ncol = 2)
    
    file_output_grid <- file.path(fig_dir, paste0("Figure_Combined_Identity_SideBySide_", env$mode, "_", task$short, ".png"))
...
saveWorkbook(wb_master_stats, file.path(tab_dir, "Summary_Individual_Identity_Stats.xlsx"), overwrite = TRUE)
cat("\n*** Percent Identity Master Pipeline Complete! Trend lines homogenized to black. ***\n")