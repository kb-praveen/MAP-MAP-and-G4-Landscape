suppressPackageStartupMessages({
  ...
  library(tidyr)
  library(ggplot2)
  library(ggrepel) # Re-added for clean motif labeling
})

# --- CONFIGURATION ---
base_path <- "Path/G4_motif_analysis/Results"
meta_path <- "Path/G4_motif_analysis/Table_S1.tsv"
...
run_comparisons <- function(input_file, ds_name) {
  ...
  for (comp in comp_list) {
    ...
    # --- LABELING LOGIC ---
    # We only want to highlight the "Strongly Enriched" and "Strongly Reduced" motifs (|Log2FC| > 1)
    label_df <- plot_df %>% filter(abs(Log2FC) > 1 & Padj < 0.05)

    p <- ggplot(plot_df, aes(x = Log2FC, y = -log10(Padj), color = Significance)) +
      geom_point(alpha = 0.6, size = 1) +
      # Motif Labels (2-fold enrichment only)
      geom_text_repel(
        data = label_df, 
...
cat("\n[SUCCESS] 8 plots generated with labeled motifs (> 2-fold change).\n")